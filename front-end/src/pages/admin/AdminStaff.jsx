...
function AdminStaff() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      console.error("L?i l?y danh s�ch staff:", err);
      alert("Kh�ng th? t?i danh s�ch k? thu?t vi�n");
    } finally {
      setLoading(false);
    }
  };
  ...
  const statusLabel = (status) => (status === "busy" ? "�ang c� l?ch" : "�ang r?nh");

  const bookingStatusMeta = (status) => {
    if (status === "completed") return { label: "Ho�n th�nh", cls: "bg-green-100 text-green-700" };
    if (status === "cancelled") return { label: "�� h?y", cls: "bg-red-100 text-red-700" };
    if (status === "in_progress") return { label: "�ang th?c hi?n", cls: "bg-amber-100 text-amber-700" };
    if (status === "confirmed") return { label: "�� x�c nh?n", cls: "bg-blue-100 text-blue-700" };
    return { label: "Ch? x�c nh?n", cls: "bg-slate-100 text-slate-700" };
  };

  const timeSlotLabel = (timeSlot) => {
    if (timeSlot === "morning") return "S�ng";
    if (timeSlot === "afternoon") return "Chi?u";
    if (timeSlot === "evening") return "T?i";
    return timeSlot;
  };
  ...
  const openScheduleModal = async (staff) => {
    ...
    try {
      ...
      console.error("L?i t?i l?ch l�m vi?c:", err);
      alert("Kh�ng th? t?i l?ch l�m vi?c");
    } finally {
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl bg-white p-5 shadow-sm">
        <h1 className="text-2xl font-semibold text-slate-800">Qu?n l� k? thu?t vi�n</h1>
        <p className="mt-1 text-sm text-slate-500">Theo d�i chuy�n m�n v� tr?ng th�i l�m vi?c c?a d?i ngu k? thu?t.</p>

        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="T�m theo t�n, s? di?n tho?i ho?c chuy�n m�n..."
          ...
          >
            ...
            <option value="available">�ang r?nh</option>
            <option value="busy">�ang c� l?ch</option>
          </select>
          ...
          <select
            ...
            className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm outline-none transition focus:border-green-600 focus:ring-2 focus:ring-green-100"
          >
            <option value="latest">M?i th�m g?n d�y</option>
            <option value="name_asc">T�n A-Z</option>
            <option value="name_desc">T�n Z-A</option>
            <option value="busy_first">Uu ti�n dang c� l?ch</option>
          </select>
      ...
      <div className="overflow-hidden rounded-xl bg-white shadow-sm">
        <table className="w-full">
          <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-500">
            <tr>
              ...
              <th className="p-3 text-left">H? T�n</th>
              <th className="p-3 text-left">S? di?n tho?i</th>
              <th className="p-3 text-left">Chuy�n m�n</th>
              <th className="p-3 text-left">Tr?ng th�i l�m vi?c</th>
              <th className="p-3 text-left">H�nh d?ng</th>
            </tr>
          </thead>
              ...
              <tr key={staff.id} className="border-b border-gray-100 text-sm">
                ...
                <td className="p-3">
                  <div className="flex flex-wrap gap-2">
                    {(staff.specialties?.length ? staff.specialties : ["Chua c?p nh?t"]).map((specialty) => (
                      <span
                        key={specialty}
            ...
            {loading && (
              <tr>
                <td colSpan="6" className="p-8 text-center text-slate-500">
                  �ang t?i danh s�ch k? thu?t vi�n...
                </td>
              </tr>
            )}
        ...
        {totalItems > 0 && (
          <div className="flex flex-col gap-3 border-t border-gray-100 px-4 py-3 text-sm text-gray-500 md:flex-row md:items-center md:justify-between">
            <p>
              Hi?n th? {fromItem} d?n {toItem} c?a {totalItems} k?t qu?
            </p>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-5xl rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-semibold text-slate-800">L?ch l�m vi?c k? thu?t vi�n</h2>
                <p className="text-sm text-slate-500">
                  {selectedStaff.name} - {selectedStaff.phone || "Chua c� s? di?n tho?i"}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="max-h-[60vh] overflow-auto rounded-xl border border-gray-100">
              <table className="w-full min-w-[900px]">
                <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-500">
                  <tr>
                    <th className="p-3 text-left">M� booking</th>
                    <th className="p-3 text-left">Kh�ch h�ng</th>
                    <th className="p-3 text-left">D?ch v?</th>
                    <th className="p-3 text-left">Ng�y</th>
                    <th className="p-3 text-left">Khung gi?</th>
                    <th className="p-3 text-left">�?a ch?</th>
                ...
                <tbody>
                  ...
                  {!scheduleLoading && scheduleRows.length === 0 && (
                    <tr>
                      <td colSpan="7" className="p-8 text-center text-slate-500">
                        K? thu?t vi�n n�y chua c� l?ch du?c ph�n c�ng.
                      </td>
                    </tr>
                  )}
                </tbody>
