...
function Bookings() {
  ...
  return (
    <section className="w-full py-8 md:py-10">
      <div className="mb-6">
        <p className="text-xs uppercase tracking-[0.14em] text-emerald-700">L?ch s?</p>
        <h1 className="mt-2 text-3xl font-bold text-slate-800">L?ch s? d?t l?ch c?a b?n</h1>
      </div>

      {bookings.length === 0 ? (
        <div className="card-soft p-8 text-center text-slate-500">B?n chua c� l?ch d?t n�o.</div>
      ) : (
        <div className="grid gap-4">
          {bookings.map((booking) => (
            ...
            >
              <div>
                <p className="text-xs uppercase tracking-[0.12em] text-slate-500">M� #{booking.id}</p>
                ...
                <div className="mt-2 flex flex-wrap gap-4 text-sm text-slate-600">
                  ...
                  <span className="font-semibold text-emerald-700">
                    {Number(booking.price || 0).toLocaleString("vi-VN")} d
                  </span>
                </div>
              </div>
